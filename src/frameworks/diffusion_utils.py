...
def sample_discrete_features(probX, probE, node_mask):
    ...
    X_t_probs = torch.gather(probX, 1, X_t.reshape(-1, 1)).reshape(bs, n)
    ...
    E_t_probs = torch.gather(probE, 1, E_t.reshape(-1, 1)).reshape(bs, n, n)
    
    total_probs_X = X_t_probs.mean(dim=1)  # Sum over nodes
    total_probs_E = E_t_probs.mean(dim=[1, 2])  # Sum over edges

    total_probs = total_probs_X + total_probs_E  # Shape [bs]

    return PlaceHolder(X=X_t, E=E_t, y=torch.zeros(bs, 0).type_as(X_t)), total_probs
