...
from src.metrics.sampling_metrics import compute_metrics
...
class MarkovBridge(pl.LightningModule):
    def __init__(
        ...
        self.model = GraphTransformer(
            ...
            act_fn_in=nn.ReLU(),
            act_fn_out=nn.ReLU(),
            # atom_vocab_size=dataset_infos.max_n_nodes,
            # atom_encoder=dataset_infos.atom_encoder,
            condition=condition
        )
    ...
    def apply_noise(self, X, E, y, X_T, E_T, y_T, node_mask, s, t_int=None):
        # Sample a timestep t.
    ...
    def forward(self, noisy_data, s, extra_data, node_mask, s_mask):
        X = torch.cat((noisy_data['X_t'], extra_data.X), dim=2).float()
        E = torch.cat((noisy_data['E_t'], extra_data.E), dim=3).float()
        y = torch.hstack((noisy_data['y_t'], extra_data.y)).float()
        return self.model(X, E, y, node_mask, s, torch.tensor(np.array(s_mask)))
    ...
    def sample_chain_no_true_no_save(self, data, batch_size, use_one_hot=False):
        ...
        for s_int in tqdm(reversed(range(0, self.T)), total=self.T):
            ...
            # Sample z_s
            sampled_s, discrete_sampled_s, node_log_likelihood, edge_log_likelihood, score = self.sample_p_zs_given_zt(
                s=s_norm,
                t=t_norm,
                X_t=X,
                E_t=E,
    ...
    def sample_chain(
        ...
        r_chain_X = torch.zeros(chain_X_size)
        r_chain_E = torch.zeros(chain_E_size)
        r_chain_X, r_chain_E = None, None
        sampled_s = None
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, X_T, E_T, y_T, node_mask, context=None, use_one_hot=False, ms=None, s_mask=None):
        ...
        logits_uncond = self.forward(noisy_data, torch.zeros_like(ms), extra_data, node_mask, s_mask)
        
        guidance_scale = 5
        
        logit_X = (1+guidance_scale) * logits_cond.X - guidance_scale * logits_uncond.X

        # Normalize predictions
        pred_X = F.softmax(logit_X, dim=-1)  # bs, n, d0
        pred_E = F.softmax(logits_cond.E, dim=-1)  # bs, n, n, d0
        # score += torch.max(logit_E, dim=-1)[0].mean(dim=[1, 2])
        # score += torch.max(logits_cond.X, dim=-1)[0].mean(dim=1)
        if use_one_hot:
            x_mask = node_mask.unsqueeze(-1).float()
            e_mask1 = x_mask.unsqueeze(2).float()
            e_mask2 = x_mask.unsqueeze(1).float()
            pred_X = F.one_hot(logit_X.argmax(dim=-1), num_classes=self.Xdim_output).float() * x_mask
            pred_E = F.one_hot(logits_cond.E.argmax(dim=-1), num_classes=self.Edim_output).float() * e_mask1 * e_mask2
